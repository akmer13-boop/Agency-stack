...
_ROP_TOOL_INSTRUCTIONS = (
    ...
    "Если пользователь просит конкретные проблемные лиды или сделки, ID карточек, "
    "кликабельные ссылки Bitrix24, разбор 'что не так' по менеджерам или детализацию "
    "компактного B2C Dashboard, обязательно используй get_rop_b2c_problem_cards. "
    "Для ссылок именно на лиды передай scope=leads; для сделок scope=deals; для обоих "
    "типов scope=all. Это текущий B2C truth-срез, а не rolling 7 дней. Не подменяй "
    "его get_rop_leads: тот инструмент возвращает агрегатную Lead Intelligence и не "
    "является источником точных First Response/Stage SLA карточек. Не проси экспорт, "
    "CSV, дополнительный доступ к CRM или шаблон URL, если get_rop_b2c_problem_cards "
    "доступен. В любой детальной выдаче всегда пиши явный тип карточки: Лид #ID или "
    "Сделка #ID; никогда не оставляй неоднозначный голый #ID. Telegram автоматически "
    "зашьёт безопасную ссылку Bitrix24 в такой номер. Сохраняй полученные tool-ссылки "
    "в ответе строго в исходном формате "
    "[Лид #ID](URL) или [Сделка #ID](URL): Telegram зашьёт ссылку в номер карточки. "
    "Не выноси тот же URL отдельной строкой. Не называй сделки лидами и "
    "не приписывай менеджерам First Response нарушения без безопасной атрибуции. "
    "Для агрегатного вопроса именно о лидах за период, статусах лидов, источниках, aging, "
    ...
    "Для B2C-вопроса 'что проверить сегодня', 'куда вмешаться' или 'какие сделки "
    "требуют внимания' обязательно используй get_rop_b2c_today_focus. Не подменяй его "
    "legacy-инструментом get_rop_focus: он смешивает B2B/B2C, показывает технические "
    "коды, суммы и старые CRITICAL-ярлыки. get_rop_focus используй только если "
    "пользователь явно запросил старый общий список по всем воронкам. В B2C-ответе "
    "сохраняй кликабельные номера сделок, не показывай CATEGORY_ID/STAGE_ID, суммы, "
    "валюты и слово 'КРИТИЧНО'. "
