...
- корректное чтение lead stage history через `STATUS_ID` / `STATUS_SEMANTIC_ID`;
- одноразовый автоматический repair старой истории лидов при первом incremental sync 0.4.13+;
- автоматическое обновление локального справочника сотрудников и подразделений вместе с Bitrix sync;
...
Lead Intelligence не считает `new_deals / new_leads` конверсией lead→deal. Успешные и неуспешные финализации лидов считаются по `lead_stage_history` и `STATUS_SEMANTIC_ID` S/F. First-response SLA по лидам пока не измеряется.
...
- Lead Data Integrity: корректные `STATUS_*`, history repair и auto-directory sync 0.4.13.
...
Начиная с 0.4.13, full и incremental sync также обновляют безопасный локальный справочник сотрудников и подразделений. Если в SQLite обнаружена старая `lead_stage_history`, сохранённая без lead-specific `STATUS_ID` / `STATUS_SEMANTIC_ID`, первый incremental sync после обновления автоматически перечитывает всю историю лидов один раз. Остальные CRM-сущности при этом остаются incremental.
...
- входы в конкретные статусы за окно;
- успешные и неуспешные финализации S/F и распределение финализаций по статусам;
...
В 0.4.13 исправлена модель полей истории: для лидов Bitrix24 используются `STATUS_ID` и `STATUS_SEMANTIC_ID`, а `STAGE_ID` / `STAGE_SEMANTIC_ID` остаются для сделок. Пока старая локальная история не repaired, Lead Intelligence явно помечает S/F как недостоверные и не выдаёт нули за бизнес-факт. После repair для каждого лида в окне учитывается последнее финальное событие, чтобы реактивация не приводила к двойному S/F.

Названия статусов и источников читаются точечно через read-only `crm.status.list`. Если справочник временно недоступен, отчёт продолжает работать и показывает CRM ID. Тексты лидов, телефоны и e-mail клиентов в AI-tool не передаются. AI-tool возвращает агрегаты и не обещает список карточек или экспорт, если отдельного инструмента для этого нет.
