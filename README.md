...
- activity-aware risk model: отдельно stage risk, подтверждённая история коммуникаций и наличие следующего шага;
- точечное read-only чтение timeline-комментариев для `/rop_deal` без сохранения их в SQLite;
- AI-tool для конкретной сделки получает агрегаты и сигналы риска без сырых текстов писем, комментариев и контактов клиента.
